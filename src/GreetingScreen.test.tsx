...
// "a button named 'Bring <name> back'" — the offer, and the one query for it: found by role and
// accessible name, so a paragraph reading those words with no button to press does not answer
// here, and neither does a control a visitor cannot reach by that name. It sits up here rather
// than inside one block for the reason rowFor and expectAgeReading already give: issue 01 asks
// whether the offer arrives and issue 02 asks whether it survives, and two spellings of "the
// offer" is how the two would drift apart. The words are copied from the Gherkin — the same
// sentence in both issues — rather than read off the component, which would pass whatever the
